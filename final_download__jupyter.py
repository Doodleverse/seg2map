...
async def async_download_tile(
    ...
    async with limit:
        ...
        # crop and download
        print(f"filename to download: {filename}")
...
async def fetch_all(session, imgs_dict: List[dict], sum_imgs: int):
    ...
    for counter, tile_dict in enumerate(imgs_dict):
        ...
        for tile_id in tile_dict["ids"]:
            ...
            print(f"multi_filename: {multi_filename}")
...
# filename = r"C:\1_USGS\5_Doodleverse\1_Seg2Map_fork\seg2map\src\seg2map\NAIP\map_less_than_1.geojson"
# with open(filename, "r") as f:
#     gpd_data = gpd.read_file(f)
# # get gpd into geojson form
# # get number of splitters need to split ROI into rectangles of 1km^2 area (or less)
# num_splitters = get_num_splitters(gpd_data)
# print("Splitting ROI into {num_splitters}x{num_splitters} tiles")
...
# roi_path = os.path.abspath(os.getcwd() + os.sep + "ROIs" + os.sep + roi_name)
...
async def download_rois(
    ...
    for roi_id in ids:
        ...
        await download_ROI(download_path,
                           gpd_data,
                           roi_id,
                           dates)
